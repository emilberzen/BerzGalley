...
namespace Tenkoku.Core
{
	...
	//private float setSize = 0.0f;
	private int px = 0;
	private int sx = 0;
	//private bool isConstellationStar = false;
	...
	if (starReset){
		...
		for (px = 0; px < StarParticles.Length; px++){
			...
			for (sx = 0; sx < (starDataArray.Length); sx++){
				...
				//StarParticles[px].position.x = setpos.x;
				//StarParticles[px].position.y = setpos.y;
				//StarParticles[px].position.z = setpos.z;
