	...
	void LateUpdate () {
		...
		if (freq > 0.0f){
			...
			if (!startLightning){
				...
				lightningFreqTime = lightningFreqTime + (Time.deltaTime/useLightningFrequency);
				...
				//timers
				useLightningSysTime = useLightningSysTime + Time.deltaTime;
				lightningTime = lightningTime + (Time.deltaTime * lightningSpeed);
				...
				if (lightningTime > 2.0f){
					lightningTime = 0.0f;
					lightningSpeed = lightningRand.Next(5.0f,12.0f);
					boltTime = boltTime + Time.deltaTime*lightningRand.Next(2.0f,10.0f);
		...
		//position
		//lightningUseVec = Quaternion.Euler(Mathf.Lerp(-20.0f,-80.0f,lightningDistance), lightningDirection + useLightningRandPos, 0.0f) * Vector3.forward;
		...
		if (lightningLight != null){
			if (lightningUseLight){
				...
				lightningLight.transform.eulerAngles = lightningLightPosition;
		...
		//-------------------------
		if (Camera.main != null){
			lightningLine.transform.position = Camera.main.transform.position;
			//lightningLine.transform.Translate(lightningUseVec * 3000.0f, Space.Self);
			lightningLine.transform.Translate(lightningUseVec * lightningRand.Next(2000f,3000f), Space.Self);
			boltPosStart = lightningLine.transform.position;
				...
				if (lightningLine != null){
					...
					for (px = 0; px < lPositions.Length; px++){
						...
						if (px > 0 && px < boltPoints-1){
							...
							//set width variance
							xR = ((px*1f)/(boltLength*1f))*Time.time;
