...
float4 skyCube = texCUBE(_Tenkoku_SkyCube,mul(_Tenkoku_CameraMV, float4(normalValues, 0)).xyz);
		...
		if (rawDepth < 1.0){
			...
			//blend height
			//fCol = lerp(fCol, sceneColor, saturate(lerp(0.0,1.0, ((dot(half3(0,1,0), i.interpolatedRay.xyz)*0.0001)))));
		...
		if (rawDepth < 1.0){
			...
			fCol.rgb = lerp(fCol.rgb, half3(0.9,1,1.1), (1-fogFac) * 2.6 * lVec * saturate(lerp(1.0,0.2,rawDepth)) * Tenkoku_LightningLightIntensity);
		...
		//tint fog
		if (rawDepth < 1.0){
			float fDist = lerp(-_Tenkoku_shaderDepth * 0.1, -_Tenkoku_shaderDepth * 5.0, _Tenkoku_FadeDistance);
			float3 useFCol = lerp(_Tenkoku_FogColor.rgb,max(max(_Tenkoku_FogColor.r,_Tenkoku_FogColor.g),_Tenkoku_FogColor.b)*0.5,saturate(_Tenkoku_overcastAmt*1.5));
			half3 sC1 = lerp(fCol.rgb, lerp(fCol.rgb,useFCol,_Tenkoku_FogColor.a)*saturate(lerp(-0.75,1.0,_Tenkoku_AmbientGI)), saturate(lerp(-500.0,0.9,rawDepth) * 0.9) );
			//fCol.rgb = lerp(sC1, skyColor.rgb, saturate(lerp(fDist,1.1,rawDepth)) );


			float diff4 = lerp(2000.0, 100.0, _Tenkoku_FadeDistance);
			float invDiff4 = abs(diff4) > 0.0001f ? 1.0 / diff4 : 0.0;
			half fogFac4 = saturate(max(0.0,usedpth) * -invDiff4 + (_tenkokufogFull * invDiff4));

			sC1 = lerp(fCol.rgb,sC1,_Tenkoku_FogDensity);
			fCol.rgb = lerp(sC1, skyColor.rgb, 1-fogFac4 );

		} else {
			//fog underside
			half hVec = saturate(lerp(-4.0,3.0,saturate(dot(half3(0,1,0), normalize( i.interpolatedRay.xyz))+0.6)));
			//fCol.rgb = lerp(skyColor.rgb,fCol.rgb,hVec);
		}
...
//fCol.rgb = skyColor.rgb;
//limit color output
//fCol.rgb = saturate(fCol.rgb);
...
//if (rawDepth >= 1.0){
//fCol = rawDepth;//max(0.0,lerp(-1.0,(1-fogFac) * 2.0,max(max(fCol.r,fCol.g),fCol.b)) * sFac);
//}
