...
Shader "TENKOKU/cloud_sphere"
{
    Properties
    {
        ...
        _overBright("OverBright", Range(0.0,10.0)) = 0.0
        [Space]
		...
		_ExtraBright("Extra brightness",Range(0,10)) = 1
    }
 ...
 float _tenkokuTimer;
        ...
        fixed4 frag(v2f i) : SV_Target{
            ...
            //float3 sky = tex2Dproj(_Tenkoku_SkyBox, UNITY_PROJ_COORD(uv0)).rgb;
            float3 sky = tex2Dproj(_Tenkoku_SkyTex, UNITY_PROJ_COORD(uv0)).rgb;
            ...
            stride = (dist1 - dist0) / samples;//(samples * max(1,1+(1-quality*0.5)) );
            offs = UVRandom(uv) * (dist1 - dist0) / samples;
            hg = HenyeyGreenstein(dot(ray, Tenkoku_Vec_SunFwd));
            hg += saturate(lerp(-3,3,hg));
            ...
            float3 retCol = sky;
            half sunDot = dot(normalize(ray), Tenkoku_Vec_SunFwd);
            half moonDot = dot(normalize(ray), Tenkoku_Vec_MoonFwd);
            float lightFac = saturate(lerp(-0.75,1.25, sunDot) );
            float lightFac2 = saturate(lerp(-8.75,1.25, sunDot) );
            float lightFac3 = saturate(lerp(0,1, sunDot+0.9) );
            float moonFac = saturate(lerp(-1,1, moonDot-0.4) );
            float moonFac2 = saturate(lerp(-1,1, moonDot-0.1) );
            ...
            //Edit Cumulus Clouds
            depth = saturate(lerp(0,8.0,depth)); //darken depth
            acc = lerp(_TenkokuCloudColor * lerp(0.6,0.45,_NoiseBias), _TenkokuCloudColor, acc.r)*_Tenkoku_AmbientGI; //tint cloud
            ...
            acc = lerp(acc, acc*_Tenkoku_overcastColor.rgb, _Tenkoku_overcastAmt);
...
acc = lerp(acc * lerp(0.7,0.4,_NoiseBias), acc, underCloud); //darken underside

        acc = lerp(lerp(_TenkokuCloudAmbientColor*0.5*_Tenkoku_Ambient, acc, saturate(underCloud + (1-lightFac3)) ), acc, _Tenkoku_AmbientGI); //under lighting (sunset)




            //Edit AltoCumulus Clouds
            acc2 = lerp(acc2, _TenkokuCloudAmbientColor*2.5, lerp(1,0.7,_TenkokuCloudAmbientColor.a) )*_Tenkoku_AmbientGI;
            acc2 = lerp(acc2, sky, 0.5 ); //blend clouds
            acc2 = lerp(acc2, _TenkokuCloudAmbientColor*2.2, depth2 * 1.5 * lightFac); //brighten front clouds
            acc2 = lerp(acc2, _TenkokuCloudAmbientColor*5.4, depth2 * lightFac2); //brighten front clouds2
                acc2 = acc2 + (depth2 * Tenkoku_MoonLightColor * 4 * moonFac * dayAmt); //moon edge glow
            ...
            acc3 = lerp(acc3 * _overBright, sky, saturate((3200.0 / ray.y) / _FarDist) ); //blend distant sky
            ...
            retCol = lerp(acc, retCol, depth); //Cumulus
            retCol = lerp(retCol, acc2, saturate(depth-depth2) ); //AltoCumulus
            retCol = lerp(retCol, lerp(retCol, acc3, saturate(depth2-depth3) ) , saturate(depth-depth3) ); //CirroStratus
            //------------------------------
            ...
            //Calculate Alpha; -------------
            half alph = depth; // cumulus
            alph = saturate(alph - saturate(depth-depth2) ); //AltoCumulus
            alph = saturate(alph - (saturate(depth-depth3) * clamp(_NoiseBias3,0,0.4)) ); //CirroStratus
            //------------------------------
            ...
            float3 ocColor = lerp(_TenkokuCloudColor.rgb, max(max(_TenkokuCloudColor.r,_TenkokuCloudColor.g),_TenkokuCloudColor.b), 0.65);
            n1 = lerp(ocColor * lerp(0.6,0.45,_NoiseBias), ocColor, saturate(lerp(0.0,1,n1.r))); //tint cloud
            //n1 = lerp(ocColor * lerp(0.6,0.45,_NoiseBias), ocColor, saturate(lerp(-0.5,1,n1.r))); //tint cloud
            n1 = lerp(n1, n1*_Tenkoku_overcastColor.rgb*lerp(1,0.5,_Tenkoku_overcastAmt), saturate(_Tenkoku_overcastAmt * 2));
            n1 = lerp(n1, sky, saturate((1900.0 / ray.y) / _FarDist) ); //blend distant sky
            retCol = lerp(retCol, n1, saturate(_Tenkoku_overcastAmt*2));
            //------------------------------
            ...
            if (Tenkoku_LightningLightIntensity > 0){
                half lDot = saturate(dot(Tenkoku_Vec_LightningFwd.xyz, normalize(ray)));
                half lVec = saturate(lDot - 0.3);
                half lVec2 = saturate(lDot - 0.65);
                half lVec3 = saturate(lDot - 0.9999);
                retCol += ((lVec*0.25) + (lVec2*0.5) + (lVec3 * 3000)) * Tenkoku_LightningLightIntensity;
            }
...
//final fog overlay
half upDot = dot(normalize(ray), half3(0,1,0));
float fogFac = saturate(lerp(0,3, upDot-0.05) );
retCol.rgb = lerp(sky, retCol.rgb, fogFac);
