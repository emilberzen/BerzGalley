...
Properties {
	...
	_Tenkoku_SkyBright ("Sky brightness", Float) = 0
...
float _Tenkoku_EclipseFactor;
		...
		v2f vert (appdata_t v)
		{
			...
			half atFac = saturate(lerp(_Tenkoku_HorizonHeight,1.0,dot(half3(0,3,0), (-eyeRay))));
			//atFac = lerp(atFac, saturate(lerp(_Tenkoku_HorizonHeight,1.0,dot(Tenkoku_Vec_SunFwd, (-eyeRay)))), 0.25);
			atFac = lerp(saturate(lerp(_Tenkoku_HorizonHeight,1.0,dot(lerp(half3(0,3,0),Tenkoku_Vec_SunFwd+half3(0.1,2.8,-0.1),_Tenkoku_Ambient), (-eyeRay)))), atFac, _Tenkoku_Ambient);

			_Tenkoku_AtmosphereDensity = lerp(_Tenkoku_AtmosphereDensity,_Tenkoku_AtmosphereDensity+(_Tenkoku_HorizonDensity),atFac);
			float kRAYLEIGH = (lerp(0, 0.0025, pow(_Tenkoku_AtmosphereDensity,2.5)));
		...
		half4 frag (v2f IN) : SV_Target
		{
			...
			col.rgb = lerp(col.rgb,half3(col.r,col.r,col.r)*_Tenkoku_Ambient,saturate(_Tenkoku_overcastColor.a*10.0));

			//sky brightness
			col.rgb *= _Tenkoku_SkyBright;

			//adjust horizon atmosphere
			half atFac = saturate(lerp(0.2,0.6,dot(half3(0,1,0), normalize(IN.rayDir.xyz))))*1.0;
			_Tenkoku_AtmosphereDensity = lerp(1.0,(lerp(0,160.0,atFac)),atFac);
			...
			if(IN.rayDir.y < 0.02){
					...
					mie = mie * lerp(0.0,1.0,_Tenkoku_EclipseFactor);
					...
					half skycoef = (lerp(-0.2,1.0,tenkoku_globalSkyColor.a));
					col = lerp(col,max(max(bcol.r,bcol.g),bcol.b),skycoef);
					...
					_TenkokuSunColor.rgb = lerp(_TenkokuSunColor.rgb,half3(1,0.25,0),atFac);
					col.rgb = col.rgb * saturate(lerp(-1.0,2.0,_Tenkoku_Ambient));
					//custom lightning
			...
			//tint sky during day
			col.rgb = lerp(col.rgb,col.rgb*_TenkokuSkyColor.rgb,_Tenkoku_Ambient);


			//test horizon atmosphere
			half atmosFac = saturate(lerp(0.15,1.0,dot(half3(0,1,0), normalize(IN.rayDir.xyz))))*2.0;
			half3 atmosCol = lerp(col.rgb, col.rgb*_Tenkoku_SkyHorizonColor.rgb*lerp(1.0,4.0,_TenkokuAmbientColor.r), atmosFac);
			col.rgb = atmosCol;
			...
			//brightness control
			col.rgb = col.rgb * lerp(1.0,1.2*_Tenkoku_SkyBright,_Tenkoku_Ambient);
			...
			//overcast tint
			//col.rgb = lerp(col.rgb,half3(col.r,col.r,col.r)*_Tenkoku_Ambient,saturate(_Tenkoku_overcastColor.a*0.5));
			col.rgb = lerp(col.rgb, max(max(col.r,col.g),col.b)*0.1, saturate(_Tenkoku_overcastAmt*2));
			...
			rainCol.rgb = lerp(rainCol,half3(2,0,0),saturate(lerp(0.0-rS,rS, rVec * 1.32))); //red
			rainCol.rgb = lerp(rainCol,half3(0,2,0),saturate(lerp(0.0-rS-0.2,rS, rVec * 1.21))); //green
			rainCol.rgb = lerp(rainCol,half3(0,0,2),saturate(lerp(0.0-rS-0.3,rS, rVec * 1.1))); //green
			rainCol.rgb = lerp(rainCol,0.0,saturate(lerp(0.0-rS-0.4,rS, rVec * 1.01)));

			//col.rgb = lerp(col.rgb, col.rgb - rainCol.r + rainCol*2, saturate(dot(Tenkoku_Vec_SunFwd.xyz, half3(0,1,0))) * (_tenkoku_rainbowFac1*0.25) );


			//overall lighting during eclipse
			col.rgb = col.rgb * saturate(lerp(-0.045,1.0,_Tenkoku_EclipseFactor));
