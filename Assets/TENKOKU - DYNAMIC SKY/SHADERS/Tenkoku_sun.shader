...
c.a += saturate(lerp(-10,1,dot(viewDir,-s.Normal))) * 0.05 * sSize * _Tenkoku_EclipseFactor;
c.a += saturate(lerp(-15,1,dot(viewDir,-s.Normal))) * 0.05 * sSize * _Tenkoku_EclipseFactor;
c.a += saturate(lerp(-20,1,dot(viewDir,-s.Normal))) * 0.1 * sSize * _Tenkoku_EclipseFactor;
c.a += saturate(lerp(-25,1,dot(viewDir,-s.Normal))) * 0.1 * sSize * _Tenkoku_EclipseFactor;
c.a += saturate(lerp(-35,1,dot(viewDir,-s.Normal))) * 0.1 * sSize * _Tenkoku_EclipseFactor;
c.a += saturate(lerp(-40,1,dot(viewDir,-s.Normal))) * sSize;



c.rgb = lerp(_CoronaColor.rgb, c.rgb * _TenkokuSunColor.rgb, c.a);
c.rgb = lerp(c.rgb,c.rgb * _TintColor.rgb,_TintColor.a);
//c.rgb = (c.rgb * (_overBright * (lerp(1,2,dot(viewDir,-s.Normal))) * saturate(lerp(-2.0,1.0,_Tenkoku_AmbientGI)) ));
